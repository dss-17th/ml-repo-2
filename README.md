...
- 특히 학습 데이터의 양이 충분하지 못할 때에는 전이학습(Transfer Learning)이 효과적이라는 사실을 확인할 수 있었습니다.  
- CLAHE를 사용한 후 전이학습을 진행 하였을 때 Accuracy가 높게 나오는 모델도 있었고, 반대로 Accuracy가 떨어지는 모델들도 존재하였습니다. 모델마다 특징을 잡기 위한 이미지 전처리 방법의 성능이 달라지는 만큼, 서로 다른 방법들을 비교하여 적절하게 적용해야 한다는 것을 배우게 되었습니다.
